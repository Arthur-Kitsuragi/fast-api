...
async def lifespan(app: FastAPI):
    ...
    model = MyModel(settings.bert_model_name, settings.lstm_units, settings.dense_units, settings.num_classes,
                    settings.nhead, settings.num_layers, settings.dropout)
...
def prepare_file(file: UploadFile) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    X_tensor = app.state.tokenizer(text, padding=True, truncation=True, return_tensors="pt")
    input_ids = X_tensor['input_ids'].to(app.state.device)
    attention_mask = X_tensor['attention_mask'].to(app.state.device)
...
def classify_pdf(input_ids, attention_mask) -> str:
    ...
    """
    try:
        preds = app.state.model(input_ids=input_ids, attention_mask=attention_mask)
        final_pred = preds.mean(dim=0).detach().cpu().numpy()
        pred_class_index = np.argmax(final_pred)
        pred_class_name = app.state.target_names[pred_class_index]
...
async def async_classify(file: UploadFile) -> str:
    ...
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(app.state.executor, lambda: model_call(input_ids, attention_mask))
